...
class NoRepeatsConstraintTest : public ::testing::Test
{
  protected:
    ...
    {
        noRepeatsConstraint =
            std::make_unique<NoRepeatsConstraint>(std::vector<Variable *>{&variables[0], &variables[1], &variables[2], &variables[3]});
    }
};
...
{
    const std::unordered_map<int, int> assignments1{{var1.getId(), 1}, {var2.getId(), 2}, {var3.getId(), 3}, {var4.getId(), 4}};
    ASSERT_TRUE(noRepeatsConstraint->isSatisfied(assignments1));

    const std::unordered_map<int, int> assignments2{{var1.getId(), 2}, {var2.getId(), 2}, {var3.getId(), 3}, {var4.getId(), 4}};
